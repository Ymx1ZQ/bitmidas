    ...
    for iii = startPlaying+1:T,
        ...
        if choices(iii-1, jjj) == -2,
            ...
            switch jjj
                ...
                case 6,
                    choices(iii, jjj) = choose006_maBaseWithSand(portfolio(1:iii-1,:,jjj), data(1:iii-1,:,:), fees, forecast(1:iii-1,:,:), stdev(1:iii-1,:,:), withinVolatility(1:iii-1,:,:), frequency);                
...
for jjj = 1:nStrategies,
    ...
    % forecast bid
    plot(forecast(startPicture-H:endPicture-H,1,H)+bandsSigmas*stdev(startPicture-H:endPicture-H,1,H), 'b-.');
    hold on;    
    plot(forecast(startPicture-H:endPicture-H,1,H)-bandsSigmas*stdev(startPicture-H:endPicture-H,1,H), 'b-.');
    hold on;        
    % forecast ask
    plot(forecast(startPicture-H:endPicture-H,2,H)+bandsSigmas*stdev(startPicture-H:endPicture-H,1,H), 'r-.');
    hold on;    
    plot(forecast(startPicture-H:endPicture-H,2,H)-bandsSigmas*stdev(startPicture-H:endPicture-H,1,H), 'r-.');
    hold on;
